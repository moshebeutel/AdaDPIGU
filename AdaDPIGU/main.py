...
def train_with_pruning(args, results_file, base_pruning_rate):
    ...
    for epoch in range(pretrain_epochs):
        ...
        eps_spent = get_epsilon(args.batchsize / args.n_training, (epoch + 1) * len(trainloader), noise_multiplier, args.delta)
        log_results(results_file, 'Stage 1 (Pretrain)', epoch + 1, train_loss, train_acc, test_loss, test_acc, eps_spent, sigma=noise_multiplier)
    ...
    for epoch in range(len(release_schedule)):
        ...
        log_results(results_file, 'Stage 2', epoch, train_loss, train_acc, test_loss, test_acc, eps_spent, sigma=noise_multiplier)
    ...
    for epoch in range(remaining_epochs):
        ...
        eps_spent = get_epsilon(args.batchsize / args.n_training, steps_so_far, noise_multiplier, args.delta)
        log_results(results_file, 'Stage 3', epoch, train_loss, train_acc, test_loss, test_acc, eps_spent, sigma=noise_multiplier)
...
def main():
    ...
    for batch_size in range(800, 1500+1, 100):
        ...
        for lr in np.arange(0.05, 0.16, 0.01):
            args.lr = round(lr, 2)
